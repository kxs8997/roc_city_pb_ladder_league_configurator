...
class SeededLadderLeague:
    ...
    def find_valid_team_arrangement(self, players, max_attempts=100):
        """Try to find a team arrangement that avoids recent matchups AND repeat partnerships.
        Returns (player_list, used_fallback) tuple."""
        ...
        # Try to find teams_needed non-overlapping pairs with no repeat partnerships
        ...
        # Try multiple times with shuffled pairs for randomness
        for attempt in range(max_attempts):
            random.shuffle(valid_pairs)
            teams = find_teams(valid_pairs, [], set())
            
            if teams is not None:
                # Check matchups (team vs team) aren't recent
                random.shuffle(teams)
                valid_matchups = True
                for i in range(0, len(teams), 2):
                    if i + 1 < len(teams):
                        team1 = frozenset(teams[i])
                        team2 = frozenset(teams[i + 1])
                        matchup = frozenset([team1, team2])
                ...
                if valid_matchups:
                    # Convert teams to flat player list for courts
                    result = []
                    for i in range(0, len(teams), 2):
                        if i + 1 < len(teams):
                            result.extend(list(teams[i]))
                            result.extend(list(teams[i + 1]))
                    return result, False
        
        # Fallback: just shuffle and return (better than no round)
        # This happens when there aren't enough valid partnerships available
        random.shuffle(players)
        return players.copy(), True
    ...
    def _generate_seeding_round(self, current_round_num):
        ...
        playing_players, used_fallback = self.find_valid_team_arrangement(playing_players)
    ...
    def _generate_tiered_round(self, current_round_num):
        """Generate a tiered round. For each tier (4-7 players):
        - Sit players who need sit-outs most (fewest sit-outs so far), avoiding consecutive.
        - Pick the first of 3 possible pairings that uses no repeat partnerships.
        - Fall back to any pairing if all have been used (warns via used_fallback flag)."""
        import itertools
        ...
        for tier_num in [1, 2, 3, 4]:
            ...
            active_courts = [c for c in assigned_courts if c <= total_courts]
            ...
            scheduled = False
            for sitters in sitter_options:
                sitter_set = set(sitters)
                active = [p for p in tier_players if p not in sitter_set]
                t1, t2 = self._pick_pairing(active)
                if t1 is not None:
                    courts.append(self._create_court_data(court_num, t1 + t2))
                    all_sitting.extend(list(sitters))
                    scheduled = True
                    break
    ...
    def _pick_pairing(self, active_players):
        ...
        Tries all 3 possible splits in random order.
        Returns (None, None) if every split reuses a partnership."""
        a, b, c, d = active_players
    ...
    def new_session(self):
        ...
        self.session_rounds = []
        self.used_partnerships = set()  # Clear partnership tracking for new session
        self.current_session += 1
    ...
    def save_to_file(self, filename):
        data = {
            ...
            'used_partnerships': [list(p) for p in self.used_partnerships]
        }
...
class EditPlayersDialog(QDialog):
    ...
    def reset_stats_keep_tiers(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            self.league.used_partnerships = set()  # Clear partnership tracking
...
class MainWindow(QMainWindow):
    ...
    def generate_9_rounds(self):
        ...
        # Different message based on session type
        if self.league.is_seeding_session:
            msg = ('Generate 9 rounds with duplicate checking?\n\n'
                   '✅ SEEDING MODE: All 28 players mix together.\n'
                   'Unique partnerships will be enforced - no repeats!')
        else:
            msg = ('Generate 9 rounds with duplicate checking?\n\n'
                   '✅ TIERED MODE: Players play within their tier.\n'
                   'Unique partnerships will be enforced - no repeats!\n\n'
                   'The algorithm will select which players sit out each round\n'
                   'to ensure no partnership repeats.')
        
        reply = QMessageBox.question(self, 'Generate 9 Rounds', msg,
                                     QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
        ...
        if errors:
            ...
            QMessageBox.information(self, 'Success', 
                                   f'✅ Generated {rounds_generated} rounds!\n\n'
                                   f'All partnerships are unique - no repeats found.')
            self.status_label.setText(f'Generated {rounds_generated} rounds - all partnerships unique!')
    
    def check_partnership_duplicates(self, from_round=None):
        """Check rounds for duplicate partnerships. Only checks rounds >= from_round if specified."""
        partnership_rounds = {}  # Map partnership -> list of round numbers
        
        for round_data in self.league.session_rounds:
            if from_round is not None and round_data['round_number'] < from_round:
                continue
            round_num = round_data['round_number']
            for court in round_data['courts']:
                # Check team1 partnership
                t1 = tuple(sorted(court['team1']))
                if t1 not in partnership_rounds:
                    partnership_rounds[t1] = []
                partnership_rounds[t1].append(round_num)
                
                # Check team2 partnership
                t2 = tuple(sorted(court['team2']))
                if t2 not in partnership_rounds:
                    partnership_rounds[t2] = []
                partnership_rounds[t2].append(round_num)
        
        # Find duplicates
        duplicates = []
        for partners, rounds in partnership_rounds.items():
            if len(rounds) > 1:
                duplicates.append((partners, rounds))
